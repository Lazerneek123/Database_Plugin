...
import com.example.demo.inputDialog.generate.InputDLiveData
...
class ActCreateLiveData : AnAction() {
    ...
    override fun actionPerformed(event: AnActionEvent) {
        ...
        if (selectedFile != null) {
            ...
            val inputDialog = InputDLiveData(directoryPath, caretOffset, psiFile, project)
            if (inputDialog.showAndGet()) {
                ...
                if (classBody == null) {
                    project.showNotification("Клас не знайдений!")
                    return
                }
                ...
                val newFields = generateLiveDataFields(
                    inputDialog.getName(),
                    inputDialog.getType(),
    ...
    private fun generateLiveDataObservationExpressions(selecteViewModel: String, name: String): List<KtExpression> {
        val psiFactory = KtPsiFactory(project)

        val observationExpression = psiFactory.createExpression(
            """
        $selecteViewModel.$name.observe(this) {
    ...
    private fun generateLiveDataFields(name: String, type: String, structure: Boolean, structureType: String): List<KtProperty> {
        val psiFactory = KtPsiFactory(project)
        ...
        val field1 = psiFactory.createDeclaration<KtProperty>(
            "val $name: LiveData<$t> = _$name"
        )

        val field2 = psiFactory.createDeclaration<KtProperty>(
            "private var _$name = MutableLiveData<$t>()"
        )
